...
def mock_gemini_model():
    """Mock Gemini model"""
    model = Mock()
    model.generate_content_async = AsyncMock(return_value=Mock(text="Test response", candidates=[]))
    return model
...
def mock_mcp_server():
    """Mock MCP server"""
    server = AsyncMock()
    server.execute_tool = AsyncMock(return_value=Mock(success=True, result={"data": "test"}))
    return server
...
def mock_rag_system():
    ...
    system.retrieve = AsyncMock(
        return_value=[
            {"content": "Test content", "similarity": 0.9, "metadata": {"source": "test.txt"}}
        ]
    )
    return system
...
async def test_research_agent_execution(
    mock_gemini_model, mock_mcp_server, mock_rag_system, mock_memory_bank
):
    ...
    task = Task(description="Test research task", domain="test", priority=Priority.HIGH)
...
async def test_validation_agent_execution(mock_gemini_model, mock_mcp_server):
    ...
    agent = ValidationAgent(
        agent_id="test_validator", gemini_model=mock_gemini_model, mcp_server=mock_mcp_server
    )

    # Mock research result
    research_result = Mock(
        task_id="task_123", content="Test content", sources=["source1", "source2"], confidence=0.9
    )
...
async def test_synthesis_agent_execution(mock_gemini_model, mock_mcp_server):
    ...
    mock_gemini_model.generate_content = Mock(
        return_value=Mock(
            text="This is a synthesized result with good structure.\n\n1. Point one\n2. Point two\n\nConclusion reached."
        )
    )

    agent = SynthesisAgent(
        agent_id="test_synthesizer", gemini_model=mock_gemini_model, mcp_server=mock_mcp_server
    )
    ...
    synthesis = await agent.execute_task(
        {"research_results": research_results, "validation_results": [], "iteration": 0}
    )
...
def test_agent_availability():
    ...
    agent = Agent(
        name="Test", agent_type=AgentType.RESEARCHER, specialization="test", max_concurrent_tasks=2
    )
