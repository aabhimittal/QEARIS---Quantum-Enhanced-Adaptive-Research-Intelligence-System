...
def sample_tasks():
    ...
    return [
        Task(description="Research quantum computing", domain="quantum", priority=Priority.HIGH),
        Task(description="Research AI systems", domain="ai", priority=Priority.MEDIUM),
        Task(description="Research NLP", domain="nlp", priority=Priority.HIGH),
    ]
...
async def test_optimization_convergence(optimizer, sample_tasks, sample_agents):
    ...
    assignment, energy_history = await optimizer.optimize_assignment(sample_tasks, sample_agents)
...
@pytest.mark.asyncio
@pytest.mark.parametrize("n_tasks,n_agents", [(3, 2), (5, 3), (10, 4)])
