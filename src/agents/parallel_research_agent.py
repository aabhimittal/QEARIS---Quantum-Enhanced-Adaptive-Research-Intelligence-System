...
class ParallelResearchAgent(BaseLLMAgent[ResearchResult]):
    ...
    async def _process_task(self, task: Task) -> ResearchResult:
        ...
        try:
            ...
            memories = await self.retrieve_memories(
                query=task.description, top_k=3, memory_type=MemoryType.EXPERIENCE
            )
            ...
            kb_results = []
            if self.rag_system:
                kb_results = await self.rag_system.retrieve(query=task.description, top_k=3)
            ...
            web_results = await self.use_tool(
                "web_search", {"query": f"{task.description} {task.domain}", "max_results": 3}
            )
            ...
            context = self._build_research_context(
                memory_context=memory_context, kb_context=kb_context, web_context=web_context
            )
            ...
            confidence = self._calculate_confidence(
                kb_results=kb_results, memories=memories, web_results=web_results
            )
            ...
            logger.error(f"[PARALLEL] {self.agent_id} research failed: {str(e)}", exc_info=True)
            ...
            return ResearchResult(
                ...
                sources=[],
                confidence=0.0,
                metadata={"error": str(e), "execution_time": execution_time, "domain": task.domain},
            )
    ...
    def _format_memories(self, memories: List[Any]) -> str:
        ...
        for i, mem in enumerate(memories, 1):
            formatted.append(f"{i}. {mem.content} " f"[Importance: {mem.importance:.2f}]")
    ...
    def _format_rag_results(self, results: List[Dict]) -> str:
        ...
        for i, result in enumerate(results, 1):
            ...
            formatted.append(f"{i}. [{source}] (relevance: {similarity:.2f})\n" f"   {content}...")
    ...
    def _build_research_context(
        self, memory_context: str, kb_context: str, web_context: str
    ) -> str:
        """
...
Be thorough but concise. Cite sources where applicable."""
    ...
    def _calculate_confidence(
        self, kb_results: List[Dict], memories: List[Any], web_results: Optional[Dict]
    ) -> float:
        """
        ...
        if kb_results:
            avg_similarity = sum(r.get("similarity", 0) for r in kb_results) / len(kb_results)
...
async def execute_parallel_research(
    agents: List[ParallelResearchAgent], tasks: List[Task]
) -> List[ResearchResult]:
    """
