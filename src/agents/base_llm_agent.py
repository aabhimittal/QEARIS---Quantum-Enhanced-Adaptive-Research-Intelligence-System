...
class BaseLLMAgent(ABC, Generic[T]):
    ...
    def __init__(
        ...
        logger.info(
            f"LLM Agent initialized: {agent_id} (type: {agent_type.value})"
        )
    ...
    async def execute_task(self, task: Any) -> T:
        ...
        try:
            ...
            logger.info(
                f"Agent {self.agent_id} completed task in {execution_time:.2f}s"
            )
            ...
            logger.error(
                f"Agent {self.agent_id} failed task: {str(e)}",
                exc_info=True
            )
            raise
        finally:
    ...
    async def _generate_response(
        ...
        try:
            ...
            if retry_count < self.config.max_retries:
                ...
                logger.error(
                    f"Gemini API call failed after {self.config.max_retries} "
                    f"attempts: {str(e)}"
                )
                raise
    ...
    async def use_tool(
        ...
        if not self.mcp_server:
            logger.warning(
                f"No MCP server configured for agent {self.agent_id}"
            )
            return None
    ...
    async def retrieve_memories(
        ...
        try:
            ...
            logger.debug(
                f"Retrieved {len(memories)} memories for query: "
                f"'{query[:50]}...'"
            )
