...
class ValidationAgent(BaseAgent):
    ...
    async def execute_task(self, result_to_validate: ResearchResult) -> ResearchResult:
        ...
        logger.info(f"{self.agent.name} validating result from {result_to_validate.agent_id}")
        ...
        try:
            ...
            validation_score = source_score * 0.4 + content_score * 0.3 + confidence_score * 0.3

            is_validated = validation_score >= 0.75

            # Generate detailed report
            report = self._generate_report(
                validation_score, source_score, content_score, confidence_score, is_validated
            )
    ...
    def _generate_report(
        self, overall: float, sources: float, content: float, confidence: float, validated: bool
    ) -> str:
...
"""
    ...
    def _generate_recommendations(
        self, overall: float, sources: float, content: float, confidence: float
    ) -> str:
