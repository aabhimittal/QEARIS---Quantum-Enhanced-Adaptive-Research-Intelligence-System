...
class ResearchAgent(BaseAgent):
    ...
    async def execute_task(self, task: Task) -> ResearchResult:
        ...
        logger.info(f"{self.agent.name} starting research: {task.description[:50]}...")
        ...
        try:
            ...
            web_results = await self.use_tool(
                "web_search", {"query": task.description, "max_results": 3}
            )
            web_context = self._format_web_results(web_results)

            # Step 4: Build comprehensive context
            context = self._build_context(memory_context, kb_context, web_context)
            ...
            confidence = self._calculate_confidence(kb_results, memories, web_results)
            ...
            await self.store_experience(
                f"Failed to research '{task.description}': {str(e)}", importance=0.5
            )
    ...
    def _format_rag_results(self, results: list) -> str:
        ...
        for result in results:
            ...
            formatted.append(f"[{source}] (relevance: {similarity:.2f})\n{content}\n")
    ...
    def _build_context(self, memory: str, rag: str, web: str) -> str:
...
Focus specifically on {task.domain} aspects and be thorough but concise."""
    ...
    def _calculate_confidence(self, kb_results: list, memories: list, web_results: Dict) -> float:
        """
    ...
    def _extract_sources(self, kb_results: list, memories: list, web_results: Dict) -> list:
