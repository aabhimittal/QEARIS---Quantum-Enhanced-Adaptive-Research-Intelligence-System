    ...
    async def _process_task(self, task_input: Any) -> ResearchResult:
        ...
        try:
            ...
            for iteration in range(self.max_iterations):
                ...
                logger.info(
                    f"[LOOP] Iteration {iterations_completed}/{self.max_iterations}"
                )
                ...
                if quality >= self.quality_threshold:
                    logger.info(
                        f"[LOOP] Quality threshold met at iteration "
            ...
            logger.error(
                f"[LOOP] {self.agent_id} synthesis failed: {str(e)}",
                exc_info=True
            )
...
Maintain all existing good content while enhancing weak areas."""
    ...
    def _calculate_quality(
        ...
        structure_count = sum(
            1 for ind in structure_indicators if ind in synthesis
        )
        ...
        source_mentions = sum(
            synthesis.lower().count(ind) for ind in source_indicators
        )
...
async def execute_synthesis_loop(
    ...
    """
    logger.info(
        f"Starting synthesis loop with {len(research_results)} research results"
    )
