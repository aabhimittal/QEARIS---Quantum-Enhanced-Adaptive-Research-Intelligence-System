...
class SynthesisAgent(BaseAgent):
    ...
    async def execute_task(self, task_input, max_iterations: int = 3) -> ResearchResult:
        """
        ...
        logger.info(f"{self.agent.name} synthesizing {len(research_results)} results")
        ...
        try:
            ...
            for iteration in range(max_iterations):
                logger.info(f"Synthesis iteration {iteration + 1}/{max_iterations}")

                # Create prompt for this iteration
                if iteration == 0:
                    prompt = self._create_initial_prompt(all_content)
                else:
                    prompt = self._create_refinement_prompt(current_synthesis, iteration)
                ...
                quality = self._calculate_quality(current_synthesis, research_results, iteration)

                final_quality = quality

                logger.info(f"Quality score: {quality:.2f} " f"(threshold: {quality_threshold})")
...
Provide an improved version that addresses these goals."""
    ...
    def _calculate_quality(
        ...
        structure_score = sum(1 for ind in structure_indicators if ind in synthesis) / len(
            structure_indicators
        )
