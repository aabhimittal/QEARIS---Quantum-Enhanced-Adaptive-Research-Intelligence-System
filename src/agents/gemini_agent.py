...
class GeminiAgent(BaseLLMAgent[ResearchResult]):
    ...
    def __init__(
        ...
        logger.info(f"Gemini Agent initialized: {agent_id} " f"(model: {self.config.model_name})")
    ...
    async def _process_task(self, task: Task) -> ResearchResult:
        ...
        logger.info(
            f"[GEMINI] {self.agent_id} processing task with Gemini " f"{self.config.model_name}"
        )
        ...
        try:
            ...
            context = self._build_gemini_context(
                task=task, memories=memories, rag_results=rag_results
            )
            ...
            # ================================================================
            confidence = self._calculate_gemini_confidence(response_text, rag_results, memories)
            ...
            logger.error(f"[GEMINI] {self.agent_id} failed: {str(e)}", exc_info=True)
    ...
    def _build_gemini_context(
        self, task: Task, memories: List[Any], rag_results: List[Dict]
    ) -> str:
        """
        ...
        if memories:
            ...
            for i, mem in enumerate(memories, 1):
                context_parts.append(f"{i}. {mem.content} [Importance: {mem.importance:.2f}]")
...
Please be thorough, accurate, and cite your sources."""
    ...
    def _calculate_gemini_confidence(
        self, response: str, rag_results: List[Dict], memories: List[Any]
    ) -> float:
        """
    ...
    def _extract_sources(self, rag_results: List[Dict], memories: List[Any]) -> List[str]:
    ...
    async def execute_with_tools(
        self, task: Task, available_tools: List[Dict[str, Any]]
    ) -> ResearchResult:
        """
        ...
        """
        logger.info(f"[GEMINI] {self.agent_id} executing with {len(available_tools)} tools")
