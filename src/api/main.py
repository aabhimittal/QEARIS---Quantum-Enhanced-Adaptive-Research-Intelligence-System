...
try:
    ...
    logger.info("[SUCCESS] Gemini API configured")
except Exception as e:
    logger.error(f"[ERROR] Failed to configure Gemini: {e}")
    sys.exit(1)
...
async def lifespan(app: FastAPI):
    # Startup
    logger.info("[START] QEARIS Starting...")
    ...
    # Shutdown
    logger.info("[STOP] QEARIS Shutting down...")
...
async def conduct_research(request: ResearchRequest):
    ...
    logger.info(f"[REQUEST] Research request: {request.query}")
...
Focus specifically on {domain} aspects."""
            ...
            results.append({"domain": domain, "content": response.text})
...
Create a comprehensive, well-structured final report."""
        ...
        logger.info(f"[SUCCESS] Research completed in {execution_time:.2f}s")
        ...
        logger.error(f"[ERROR] Research failed: {str(e)}")
