...
class AgentEvaluator:
    ...
    def evaluate_agent(
        self, agent: Agent, results: List[ResearchResult] = None
    ) -> EvaluationResult:
        """
        ...
        recommendations = self._generate_recommendations(scores, weaknesses, agent.specialization)
        ...
        logger.info(f"Evaluated {agent.id}: " f"Score={overall_score:.2f}, Grade={grade}")
    ...
    def _calculate_success_rate(self, results: List[ResearchResult]) -> float:
        ...
        successful = sum(
            1 for r in results if r.confidence > 0.7 and r.sources and "error" not in r.metadata
        )
    ...
    def _calculate_speed_score(self, results: List[ResearchResult]) -> float:
        ...
        """
        times = [r.metadata.get("execution_time", 30) for r in results]
    ...
    def _calculate_quality_score(self, results: List[ResearchResult]) -> float:
        ...
        # Combined quality
        quality = avg_confidence * 0.5 + source_score * 0.3 + length_score * 0.2
    ...
    def _generate_recommendations(
        self, scores: Dict[str, float], weaknesses: List[str], specialization: str
    ) -> List[str]:
        ...
        if scores["success_rate"] < 0.7:
            ...
            recommendations.append("Verify source credibility before using")

        # Speed recommendations
        if scores["speed"] < 0.7:
            recommendations.append("Optimize tool usage and reduce redundant searches")
            recommendations.append("Cache frequently accessed information")

        # Quality recommendations
        if scores["quality"] < 0.7:
            recommendations.append("Use more diverse and credible sources")
            recommendations.append(f"Deepen expertise in {specialization} domain")

        # Consistency recommendations
        if scores["consistency"] < 0.7:
            recommendations.append("Standardize research methodology")
            recommendations.append("Improve prompt engineering for stable outputs")

        # General recommendations
        if not recommendations:
            recommendations.append("Maintain current performance level")
            recommendations.append("Consider taking on more complex tasks")
