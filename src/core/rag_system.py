...
class RAGSystem:
    ...
    def __init__(
        self, config: Any = None, chunk_size: int = 512, chunk_overlap: int = 50, top_k: int = 5
    ):
        """
    ...
    async def _load_initial_knowledge(self):
        ...
        for doc in initial_docs:
            await self.add_document(content=doc["content"], metadata={"source": doc["source"]})
    ...
    async def add_document(self, content: str, metadata: Optional[Dict[str, Any]] = None) -> str:
        ...
        logger.debug(
            f"Added document: {len(chunks)} chunks, " f"source={metadata.get('source', 'unknown')}"
        )
    ...
    async def retrieve(self, query: str, top_k: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        for doc, sim in similarities[:k]:
            results.append(
                {"content": doc.content, "similarity": float(sim), "metadata": doc.metadata}
            )

        logger.debug(f"Retrieved {len(results)} documents for query: " f"'{query[:50]}...'")
